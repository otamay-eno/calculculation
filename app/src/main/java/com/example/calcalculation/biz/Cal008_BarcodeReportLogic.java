...
/**
 * 【機能】データベース操作<br>
 * 【概要】バーコードレポート画面のビジネスロジッククラスです<br>
 * 【作成日、作成者】2024/01/31 N.OONISHI
 */
public class Cal008_BarcodeReportLogic extends CalBaseBusinessLogic {

    /**
     * 【機能】非同期更新処理<br>
     * 【概要】本アプリの基底Logicクラスを呼び出し非同期でバーコードに紐づく全食べ物名・カロリー検索処理を行います<br>
     * 【作成日、作成者】2024/01/31 N.OONISHI
     *
     * @param context データベース操作を実行したいクラスのコンテキスト
     * @param params   実行したいSQLのパラメーター
     */
    public Single<List<Barcode>> getBarcodeFoodCal(Context context, Object... params) {
        CalBaseBusinessLogic biz = new CalBaseBusinessLogic();
        return biz.executeBarcodeAsync(context, CalConst.SQL_GET_FOODCAL, params)
                .map(list -> {
